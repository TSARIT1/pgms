...
export default function PaymentListPage() {
  ...
  const mapPaymentToRow = (p) => ({
    ...
    Type: (p.paymentType || 'RENT_PAYMENT').replace('_', ' '),
    __raw: p,
  })
  ...
  const handleAddPayment = async (paymentData) => {
    ...
    try {
      ...
      const payload = {
        ...
        gstPercentage: paymentData.gstPercentage,
  ...
  const handleDownload = (payment) => {
    ...
    import('jspdf').then(({ default: jsPDF }) => {
      ...
      // GST details calculation
      const totalAmount = Number(String(payment.Amount).replace(/\s+/g, ''))
      const gstPercentage = payment.__raw ? (payment.__raw.gstPercentage || 0) : 0
      ...
      doc.setFont('helvetica', 'normal')
      doc.text(baseAmount.toFixed(2), 75, yPos)
      
      yPos += 10
      doc.setFont('helvetica', 'bold')
      doc.text('GST %:', 25, yPos)
      doc.setFont('helvetica', 'normal')
      doc.text(`${gstPercentage}%`, 75, yPos)

      yPos += 10
      doc.setFont('helvetica', 'bold')
      doc.text('GST Amount:', 25, yPos)
      doc.setFont('helvetica', 'normal')
      doc.text(gstAmount.toFixed(2), 75, yPos)
      ...
      doc.setFontSize(14)
      doc.setFont('helvetica', 'bold')
      doc.text(totalAmount.toFixed(2), 75, yPos)
