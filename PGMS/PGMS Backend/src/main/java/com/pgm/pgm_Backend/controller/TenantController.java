...
public class TenantController {
    ...
    public ResponseEntity<?> getAllTenantsIncludingDeleted() {
        try {
            List<Tenant> tenants = tenantService.getAllTenantsIncludingDeleted();
            Map<String, Object> response = new HashMap<>();
            response.put("status", "success");
            response.put("message", "All tenants including deleted retrieved successfully");
            response.put("data", tenants);
            response.put("count", tenants.size());
            return ResponseEntity.ok(response);
        } catch (Exception e) {
