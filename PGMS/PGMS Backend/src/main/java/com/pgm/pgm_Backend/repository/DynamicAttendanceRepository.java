...
public class DynamicAttendanceRepository {
    ...
    public Optional<Attendance> findById(Long adminId, Long id) {
        ...
        List<Attendance> results = jdbcTemplate.query(sql, new AttendanceRowMapper(), id);
    ...
    public List<Attendance> findByStudentName(Long adminId, String studentName) {
        String sql = "SELECT * FROM " + getTableName(adminId) + " WHERE student_name = ?";
    ...
    public List<Attendance> findByDateBetween(Long adminId, LocalDate startDate, LocalDate endDate) {
        String sql = "SELECT * FROM " + getTableName(adminId) + " WHERE date BETWEEN ? AND ?";
        return jdbcTemplate.query(sql, new AttendanceRowMapper(), startDate, endDate);
    }

    public Optional<Attendance> findByStudentNameAndDate(Long adminId, String studentName, LocalDate date) {
        String sql = "SELECT * FROM " + getTableName(adminId) + " WHERE student_name = ? AND date = ?";
        List<Attendance> results = jdbcTemplate.query(sql, new AttendanceRowMapper(), studentName, date);
        return results.isEmpty() ? Optional.empty() : Optional.of(results.get(0));
    }
