...
public class DynamicTenantRepository {
    ...
    public List<Tenant> findAll(Long adminId) {
        String sql = "SELECT * FROM " + getTableName(adminId) + " WHERE is_deleted = false OR is_deleted IS NULL";
        return jdbcTemplate.query(sql, new TenantRowMapper());
    }

    public List<Tenant> findAllIncludingDeleted(Long adminId) {
    ...
    private Tenant insert(Long adminId, Tenant tenant) {
        String sql = "INSERT INTO " + getTableName(adminId) +
                ...
                "identity_proof_type, identity_proof, status, is_deleted, created_at, updated_at) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        Boolean isDeleted = tenant.getIsDeleted() != null ? tenant.getIsDeleted() : false;
    ...
    private Tenant update(Long adminId, Tenant tenant) {
        String sql = "UPDATE " + getTableName(adminId) +
                ...
                "status = ?, is_deleted = ?, updated_at = ? WHERE id = ?";
        ...
        jdbcTemplate.update(sql,
                ...
                tenant.getStatus(),
                tenant.getIsDeleted() != null ? tenant.getIsDeleted() : false,
                now,
                tenant.getId());
