...
public class DynamicTableService {
    ...
    /**
     * Migration: Add is_deleted column to existing tenant tables
     */
    private void migrateAddIsDeletedColumn(Long adminId) {
        String tableName = "admin_" + adminId + "_tenants";
        try {
    ...
    /**
     * Migration: Add gst_percentage and payment_type columns to existing payment
     * tables
     */
    private void migrateAddPaymentColumns(Long adminId) {
        String tableName = "admin_" + adminId + "_payments";
        try {
            // Check if gst_percentage column exists
            String checkSql = "SELECT COUNT(*) FROM information_schema.columns " +
                    "WHERE table_schema = DATABASE() AND table_name = ? AND column_name = 'gst_percentage'";
            Integer count = jdbcTemplate.queryForObject(checkSql, Integer.class, tableName);

            if (count == null || count == 0) {
                String alterSql = "ALTER TABLE " + tableName +
                        " ADD COLUMN gst_percentage DOUBLE DEFAULT 0.0, " +
                        " ADD COLUMN payment_type VARCHAR(50) DEFAULT 'RENT_PAYMENT'";
                jdbcTemplate.execute(alterSql);
                System.out.println("✓ Added payment columns to " + tableName);
            }
        } catch (Exception e) {
    ...
    /**
     * Migration: Add identity_proof_type and identity_proof columns to existing
     * tenant tables
     */
    private void migrateAddIdentityProofColumns(Long adminId) {
        String tableName = "admin_" + adminId + "_tenants";
        try {
            // Check if identity_proof_type column exists
            String checkSql = "SELECT COUNT(*) FROM information_schema.columns " +
                    "WHERE table_schema = DATABASE() AND table_name = ? AND column_name = 'identity_proof_type'";
            Integer count = jdbcTemplate.queryForObject(checkSql, Integer.class, tableName);

            if (count == null || count == 0) {
                String alterSql = "ALTER TABLE " + tableName +
                        " ADD COLUMN identity_proof_type VARCHAR(100), " +
                        " ADD COLUMN identity_proof LONGTEXT";
                jdbcTemplate.execute(alterSql);
                System.out.println("✓ Added identity proof columns to " + tableName);
            }
        } catch (Exception e) {
            System.err.println("Warning: Could not add identity proof columns to " + tableName + ": " + e.getMessage());
        }
    }
